...
The main purpose for a command like this would be for something that can happen everywhere and is not tied to a specific world. For example a `serverRules` command to tell the user thats running the command in chat what are the servers rules are nomatter what world they are in.
...
```
```java
public class RulesCommand excends AbstractPlayerCommand {
